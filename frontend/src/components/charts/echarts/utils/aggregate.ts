...
// utils functions for aggregate transform
...
class ResultDimInfoInternal {
  ...
  gatherValue(
    groupByDimInfo: ExternalDimensionDefinition | undefined,
    groupVal: OptionDataValue | undefined,
...
type CreateInTravel<LINE> = (
  ...
  groupByDimInfo: ExternalDimensionDefinition | undefined,
  groupByVal: OptionDataValue | undefined
) => LINE;
...
function prepareDimensions(
  ...
  for (let i = 0; i < resultDimensionsConfig.length; i++) {
    ...
    const dimInfoInUpstream = upstream.getDimensionInfo(
      resultDimInfoConfig.from
    );
    assert(
      dimInfoInUpstream,
      "Can not find dimension by `from`: " + resultDimInfoConfig.from
    );
    ...
    assert(
      !groupByDimInfo ||
        groupByDimInfo.index !== dimInfoInUpstream.index ||
        rawMethod == null,
    ...
    const name =
      resultDimInfoConfig.name != null
        ? resultDimInfoConfig.name
        : (dimInfoInUpstream.name as DimensionName);
    ...
    if (hasOwn(METHOD_NEEDS_COLLECT, method)) {
      needCollect = true;
      const collectionTargetMethods =
        METHOD_NEEDS_COLLECT[method as keyof typeof METHOD_NEEDS_COLLECT];
...
function prepareGroupByDimInfo(
  config: AggregateTransformOption["config"],
  upstream: ExternalSource
): ExternalDimensionDefinition | undefined {
  const groupByConfig = config.groupBy;
  let groupByDimInfo: ExternalDimensionDefinition | undefined;
  if (groupByConfig != null) {
    groupByDimInfo = upstream.getDimensionInfo(groupByConfig);
    assert(
      groupByDimInfo,
      "Can not find dimension by `groupBy`: " + groupByConfig
    );
  }
...
function travel<LINE>(
  ...
  if (groupByDimInfo) {
    ...
    for (
      let dataIndex = 0, len = upstream.count();
      dataIndex < len;
      dataIndex++
    ) {
      const groupByVal = upstream.retrieveValue(
        dataIndex,
    ...
    for (
      let dataIndex = 1, len = upstream.count();
      dataIndex < len;
      dataIndex++
    ) {
      doUpdate(
...
function isGroupByDimension(
  ...
  return (
    groupByDimInfo !== undefined &&
    targetDimInfo.indexInUpstream === groupByDimInfo.index
  );
}
