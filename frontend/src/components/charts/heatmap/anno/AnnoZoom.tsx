"use client";

import { useMemo } from "react";
import { Box } from "@mui/material";
import { BaseDynamicParams } from "../../ggplot2/common/BaseDynamicParams";
import { ParameterConfig } from "../../ggplot2/types";

export interface AnnoZoomConfig {
  type: "anno_zoom";
  arguments?: {
    [key: string]: any;
  };
}

interface AnnoZoomComponentProps {
  value?: AnnoZoomConfig;
  onChange?: (config: AnnoZoomConfig) => void;
}

/**
 * AnnoZoom 组件
 * Zoom annotation for ComplexHeatmap
 */
export const AnnoZoom: React.FC<AnnoZoomComponentProps> = ({
  value,
  onChange,
}) => {
  const availableParams: ParameterConfig[] = useMemo(
    () => [
      {
        name: "link_width",
        type: "unit",
      },
      {
        name: "link_height",
        type: "unit",
      },
      {
        name: "link_gp",
        type: "gpar",
      },
      {
        name: "link_rot",
        type: "number",
        default: 0,
        min: 0,
        max: 360,
      },
      {
        name: "which",
        type: "select",
        options: ["column", "row"],
        default: "column",
      },
    ],
    []
  );

  const paramsValue = useMemo(() => {
    if (!value || !value.arguments) {
      return {};
    }
    return value.arguments;
  }, [value]);

  const handleChange = (params: Record<string, any>) => {
    if (!onChange) return;
    onChange({
      type: "anno_zoom",
      arguments: params,
    });
  };

  return (
    <Box>
      <BaseDynamicParams
        availableParams={availableParams}
        value={paramsValue}
        onChange={handleChange}
        nested={true}
      />
    </Box>
  );
};

