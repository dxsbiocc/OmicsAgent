...
#                         数据操作
# ========================================================
group_col <- cfg$qcorrplot$arguments$group
if (is.null(group_col) || length(unique(data[[group_col]])) != 2) {
    stop("group column error!")
}
splited_data <- split(
    dplyr::select(data, !all_of(group_col)),
    data[[group_col]]
)

cfg$correlate$x <- splited_data[[1]]
new_data <- do.call(correlate, cfg$correlate) %>%
    as_cor_tbl(
        extra.mat = list(r2 = cor(splited_data[[2]]))
    )
...
for (i in seq_along(cfg$layers)) {
    ...
    if (!is.null(dtype)) {
        cfg$layers[[i]]$arguments$data <- get_data(type = dtype)
