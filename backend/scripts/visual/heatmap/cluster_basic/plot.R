...
if (is.null(params$heatmap)) {
    stop("heatmap configuration is required. Please provide 'heatmap' parameter in JSON.") # nolint: line_length_linter.
}

# 加载 build_heatmap 函数库
utils_path <- file.path(script_root, "scripts", "utils", "build_heatmap.R")
if (!file.exists(utils_path)) {
    stop(paste("build_heatmap.R not found at:", utils_path))
}
source(utils_path)
# ========================================================
#                         参数读取
...
#                         数据操作
# ========================================================
mat <- transform_matrix(data$matrix, cfg$transform)

# ========================================================
#                         图形绘制
# ========================================================
p <- build_heatmap(cfg, mat)
