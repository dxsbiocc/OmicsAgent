...
# heatmap
tile_index <- get_index_or_stop(cfg$layers, equal = "geom_tile")

cate_col <- cfg$layers[[tile_index]]$mapping$y
if (is.null(cate_col)) {
    stop()
}
mat_data <- data %>%
    group_by(!!sym(x_axis), !!sym(cate_col)) %>%
    summarise(mu = mean(!!sym(y_axis))) %>%
    mutate(
        !!cate_col := factor(.data[[cate_col]],
            levels = unique(.data[[cate_col]])
        ),
        index = as.numeric(!!sym(cate_col))
    )
cfg$layers[[tile_index]]$arguments$data <- mat_data
cfg$layers[[tile_index]]$mapping$y <- "index"
# violin
violin_index <- get_index_or_stop(cfg$layers, equal = "geom_violin")
cate_names <- unique(data[[cate_col]])
add_high <- length(cate_names) + 1
cfg$layers[[violin_index]]$mapping$y <- glue::glue("{y_axis} + {add_high}")

y_max <- ceiling(max(data[[y_axis]])) + add_high
y_min <- floor(min(data[[y_axis]])) + add_high
# geom_textpath
textpath_index <- get_index_or_stop(cfg$layers, equal = "geom_textpath")
cfg$layers[[textpath_index]]$mapping$y <- y_max + 0.5
...
arguments <- cfg$layers[[annotate_index[1]]]$arguments
if (is.null(arguments$x)) {
    cfg$layers[[annotate_index]]$arguments$x <- c(0.5, 0.5)
}
if (is.null(arguments$xend)) {
    cfg$layers[[annotate_index]]$arguments$xend <- c(x_size + 0.5, 0.5)
}
if (is.null(arguments$y)) {
    cfg$layers[[annotate_index]]$arguments$y <- c(y_max + 0.5, add_high)
}
if (is.null(arguments$yend)) {
    cfg$layers[[annotate_index]]$arguments$yend <- c(y_max + 0.5, y_max)
}
# fill manual
...
# fill_gradientn
fill_gradientn <- get_index_or_stop(cfg$scales, equal = "scale_fill_gradientn")
col_values <- cfg$scales[[fill_gradientn]]$arguments$colours
cfg$scales[[fill_gradientn]]$arguments$colours <- colorRampPalette(col_values)(50) # nolint: line_length_linter.
# scale_y_continuous
...
if (is.null(arguments$limits)) {
    cfg$scales[[y_continuous]]$arguments$limits <- c(-1, y_max + 0.5)
}

y_labels <- as.integer(seq(add_high, y_max, length.out = 4))
if (is.null(arguments$breaks)) {
    cfg$scales[[y_continuous]]$arguments$breaks <- c(
        seq_along(cate_names),
        y_labels
    )
}
if (is.null(arguments$labels)) {
    cfg$scales[[y_continuous]]$arguments$labels <- c(
        cate_names,
        y_labels - add_high
    )
}
