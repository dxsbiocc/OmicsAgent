...
text_index <- which(sapply(cfg$layers, function(x) {
    !is.null(x$type) && x$type == "geom_label_repel"
}))
if (length(text_index) > 0) {
    topk <- cfg$layers[[text_index]]$arguments$top.k %||% 10
    cfg$layers[[text_index]]$arguments$top.k <- NULL
    log2_fc <- cfg$mapping$y
    group <- cfg$layers[[text_index]]$mapping$colour
    text_data <- dplyr::filter(data, str_to_lower(!!sym(group)) != "none") %>%
        group_by(!!sym(log2_fc) < 0) %>%
        top_n(topk, abs(!!sym(log2_fc)))
    ...
    for (i in point_index) {
        if (is.null(cfg$layers[[i]]$arguments$top.k)) {
            next
        }
