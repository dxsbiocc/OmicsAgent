...
angle <- 90 - 360 * (seq_len(nrow(data)) - 0.5) / nrow(data)

annotate_index <- which(sapply(cfg$layers, function(x) {
    !is.null(x$type) && x$type == "annotate"
}))
for (i in annotate_index) {
    if (cfg$layers[[i]]$arguments$geom == "rect") {
        cfg$layers[[i]]$arguments$xmin <- as.numeric(cfg$layers[[i]]$arguments$xmin) # nolint: line_length_linter.
        cfg$layers[[i]]$arguments$xmax <- as.numeric(cfg$layers[[i]]$arguments$xmax) # nolint: line_length_linter.
        cfg$layers[[i]]$arguments$ymin <- as.numeric(cfg$layers[[i]]$arguments$ymin) # nolint: line_length_linter.
        cfg$layers[[i]]$arguments$ymax <- as.numeric(cfg$layers[[i]]$arguments$ymax) # nolint: line_length_linter.
    }
}
...
for (i in text_index) {
    angle_just <- cfg$layers[[i]]$arguments$angle
    cfg$layers[[i]]$arguments$angle <- ifelse(angle < -90, angle + 180, angle + angle_just) # nolint: line_length_linter.

    hjust <- cfg$layers[[i]]$arguments$hjust %||% 0.5
    cfg$layers[[i]]$arguments$hjust <- ifelse(angle < -90, 1 + hjust, -1 * hjust) # nolint: line_length_linter.
}
