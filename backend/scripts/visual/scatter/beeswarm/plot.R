...
reorder_var <- function(df, cat, num) {
    reorder(df[[cat]], df[[num]])
}
...
if (is.numeric(data[[cfg$mapping$x]])) {
    num_col <- cfg$mapping$x
    cate_col <- cfg$mapping$y
} else {
    num_col <- cfg$mapping$y
    cate_col <- cfg$mapping$x
}

text_data <- group_by(data, across(all_of(c(cate_col, group_col)))) %>%
    reframe(
        y = max(!!sym(num_col)),
        num = n(),
        first = quantile(!!sym(num_col), .25),
        second = quantile(!!sym(num_col), .5),
        third = quantile(!!sym(num_col), .75),
    )
for (i in seq_len(length(cfg$layers))) {
    if (cfg$layers[[i]]$type == "geom_quasirandom") {
        next
    }
