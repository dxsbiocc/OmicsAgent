...
if (is.numeric(data[[cfg$mapping$x]])) {
    ...
    group_col <- cfg$mapping$y
} else {
    num_col <- cfg$mapping$y
    group_col <- cfg$mapping$x
}

rect_data <- data %>%
    pivot_wider(
        id_cols = all_of(group_col),
        names_from = all_of(cfg$mapping$colour %||% cfg$mapping$fill),
        values_from = all_of(num_col)
    )
rect_index <- which(sapply(cfg$layers, function(x) {
    !is.null(x$type) && x$type == "geom_rect"
}))
cols <- colnames(rect_data)
cfg$layers[[rect_index]]$mapping$ymin <- cols[2]
cfg$layers[[rect_index]]$mapping$ymax <- cols[3]
cfg$layers[[rect_index]]$arguments$data <- rect_data
