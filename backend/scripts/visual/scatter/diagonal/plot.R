...
for (i in top_index) {
    ...
    x_col <- all.vars(parse_expr(cfg$mapping$x))
    y_col <- all.vars(parse_expr(cfg$mapping$y))
    group <- cfg$mapping$colour

    text_data <- data %>%
        dplyr::filter(str_to_lower(!!sym(group)) != "none") %>%
        mutate(log2FC = !!parse_expr(glue::glue("log2({x_col}/{y_col})"))) %>%
        group_by(log2FC > 0) %>%
        top_n(topk, abs(log2FC))
