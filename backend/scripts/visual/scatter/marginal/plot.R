...
marginal_index <- which(sapply(cfg$layers, function(x) {
    !is.null(x$type) && x$type == "ggMarginal"
}))
if (length(marginal_index) > 0) {
    args <- cfg$layers[[marginal_index]]$arguments
    cfg$layers[[marginal_index]] <- NULL

    p <- build_ggplot(cfg, data)

    args$p <- p
    p <- suppressWarnings(do.call(ggMarginal, args))
} else {
    p <- build_ggplot(cfg, data)
}
# ========================================================
