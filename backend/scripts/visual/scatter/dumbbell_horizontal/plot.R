...
# 处理文本位置
col <- cfg$mapping$colour %||% cfg$mapping$fill
groups <- unique(data[[col]])
text_index <- which(sapply(cfg$layers, function(x) {
    !is.null(x$type) && x$type == "geom_text"
}))
hjust <- cfg$layers[[text_index]]$arguments$hjust %||% 0.5
cfg$layers[[text_index]]$arguments$hjust <- ifelse(data[[col]] == groups[1], 1 + hjust, -1 * hjust) # nolint: line_length_linter.
