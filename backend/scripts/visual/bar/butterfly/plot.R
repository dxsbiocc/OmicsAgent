...
new_data <- data %>%
    mutate(
        !!y_axis := factor(.data[[y_axis]], levels = unique(.data[[y_axis]])),
        !!group := factor(.data[[group]], levels = cates),
        !!x_axis := ifelse(.data[[group]] == cates[1], -.data[[x_axis]], .data[[x_axis]]) # nolint: line_length_linter.
    )
...
# convert to grob
gt <- ggplotGrob(p)
axis_layout <- gt$layout[gt$layout$name == "axis-l-1-1", ]
# remove left y-axis
idx <- which(gt$layout$name == "axis-l-1-1")
gt$grobs[[idx]] <- nullGrob()
gt$widths[axis_layout$l] <- unit(0, "cm")
