...
# ========================================================
n <- dim(data)[1]
fill_index <- which(sapply(cfg$scales, function(x) {
    !is.null(x$type) && startsWith(x$type, "scale_fill")
}))
color_list <- cfg$scales[[fill_index]]$arguments$values
if (length(color_list) < n) {
    cfg$scales[[fill_index]]$arguments$values <- colorRampPalette(color_list)(n)
}
# ========================================================
#                         图形绘制
# ========================================================
