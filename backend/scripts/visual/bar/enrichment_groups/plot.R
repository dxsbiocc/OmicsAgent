...
new_data <- data %>%
    arrange(!!sym(y_axis)) %>%
    mutate(
        !!y_axis := factor(.data[[y_axis]], levels = .data[[y_axis]]),
        !!group := factor(.data[[group]], levels = unique(.data[[group]]))
    )

# left geom
point_index <- which(sapply(cfg$layers, function(x) {
    !is.null(x$mapping$x) & x$type == "geom_point" & is.numeric(x$mapping$x)
}))

if (length(point_index) > 0) {
    width <- abs(cfg$layers[[point_index]]$mapping$x)
} else {
    width <- 0.5
}
rect_data <- group_by(new_data, !!sym(group)) %>%
    reframe(n = n()) %>%
    ungroup() %>%
    arrange(!!sym(group)) %>%
    mutate(
