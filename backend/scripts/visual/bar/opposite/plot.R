...
group_col <- all.vars(parse_expr(cfg$mapping$fill))
groups <- unique(data[[group_col]])
if (length(groups) != 2) {
    stop("Only support two groups!")
}
new_data <- data %>%
    mutate(
        sign = ifelse(!!sym(group_col) == groups[1], 1, -1),
        margin = ifelse(sign == 1, -0.2, 0.2),
        hjust = ifelse(sign == 1, 1, 0)
    ) %>%
    group_by(!!sym(group_col)) %>%
    arrange(!!parse_expr(cfg$mapping$x)) %>%
    mutate(!!y_axis := factor(.data[[y_axis]], levels = .data[[y_axis]]))

cfg$mapping$x <- paste0(cfg$mapping$x, "*sign")
# ========================================================
#                         图形绘制
# ========================================================
